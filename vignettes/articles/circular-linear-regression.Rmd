---
title: "Circular–linear regression on the cylinder"
---
...
**C–L** · [L–C](linear-circular-regression.html) ·
...
article](circular-circular-regression.html): **`vmlss()`** leads —
mean direction (tan-half link) and concentration (log link) are
...
cannot follow it across $\pm\pi$; **`pnlss()`** can.
...
The two estimated smooths on the link scale — this per-parameter
readout (location *and* concentration, each with its own credible band)
is the distributional-regression payoff:

```{r smooths-cl, fig.height = 4}
plot(b, pages = 1, scheme = 1)
```

## Flat view

The 95% band for $\mu(x)$ is the link-scale interval pushed through the
monotone tan-half link:
...
$(x, \theta)$ maps to $\big(a(x),\ r\cos\theta,\ r\sin\theta\big)$ with
the covariate stretched along the axis. Same toolkit as the torus
article: `persp()` for the projection matrix, `trans3d()` for drawing,
depth-based fading, and the 95% band as a translucent ribbon (the
dashed axial line is $\theta = 0$, the reference direction):
...
```{r cylinder-vm, fig.height = 4.6}
draw_cylinder(dat, mu_hat, xg, lo = mu_lo, hi = mu_hi)
```

The fitted mean direction swings around the reference line and back —
it never wraps the tube, which is exactly the class the tan-half link
can represent.
...
ordinary smooths, $\mu_1(x)$ and $\mu_2(x)$, whose angle winds freely:

```{r helix, fig.height = 4.6}
set.seed(20260612)
n <- 300
x2 <- runif(n)
...
The same trade as on the torus applies — `pnlss` buys unbounded
rotation at the price of entangling direction and concentration in
$(\mu_1, \mu_2)$.
