...
must agree within pinned tolerances. At v0.0.1, the battery agrees to
machine precision (~1e-11) for parametric and cyclic-spline models, and to
~3e-5 (smoothing parameters, log-likelihood) with fitted curves within
2e-6 radians for thin-plate models, where basis eigen-decomposition makes
bitwise agreement impossible. The REML criteria match to ~1e-9 after
normalizing conventions (hea reports -2 log REML, mgcv -log REML).

`dev/parity/run.sh` reruns the whole side-by-side comparison; the frozen
battery is asserted hermetically in `tests/testthat/test-vmlss-parity.R`.
