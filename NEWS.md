...
* The differential battery grows to twelve cases; all green. One honest
  reclassification: `wc_cyclic` sits in the EIGEN tolerance class despite
  its deterministic cc basis, because the heavy-tailed likelihood makes
  the REML surface flat at the selected lambda — both engines agree on
  the criterion to ~1e-8 (same optimum) while stopping-point noise leaves
  ~1e-5 in lambda/edf/loglik. TIGHT requires both a deterministic basis
  and a well-conditioned criterion.
